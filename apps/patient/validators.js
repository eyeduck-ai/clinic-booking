...
export function normalizeTWId(value) {
  if (typeof value !== "string") return "";
  return value.trim().toUpperCase();
}

export function isValidTWId(value, { allowResident89 = true } = {}) {
  ...
  const digits = input.slice(1).split("").map((d) => Number(d));
...
export function normalizeBookingRef(value) {
  if (typeof value !== "string") return "";
  return value.replace(/\s+/g, "").toUpperCase();
}

export function isValidBookingRef(value) {
...
export function generateBookingRef(length = BOOKING_REF_LENGTH) {
